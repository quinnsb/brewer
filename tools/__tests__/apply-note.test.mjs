...
function fullBuild(notes = {}) {
  ...
  return applyPalette(catalog.map((item) => mergeItem(item, notes[item.id] ?? null, false, () => {})), palette);
}
...
test("saving a note matches what the full build would produce", () => {
  ...
  const viaAdmin = itemWithNote({ items: published.items, rawItems: raw.items, taxonomy, listening, palette, id: ID, noteText });
...
test("clearing a note matches the full build too", () => {
  const viaAdmin = itemWithNote({ items: published.items, rawItems: raw.items, taxonomy, listening, palette, id: ID, noteText: null });
...
test("every other item is left byte-identical", () => {
  ...
  const { items } = itemWithNote({ items: published.items, rawItems: raw.items, taxonomy, listening, palette, id: ID, noteText });
...
test("item order is preserved", () => {
  const { items } = itemWithNote({ items: published.items, rawItems: raw.items, taxonomy, listening, palette, id: ID, noteText: null });
...
test("a note buildNote produces round-trips through the merge", () => {
  ...
  const { item } = itemWithNote({ items: published.items, rawItems: raw.items, taxonomy, listening, palette, id: ID, noteText: note });
