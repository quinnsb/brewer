/* ============================================================
   LIBRARY MOTION LAB — shelves and immersive media detail

   A copy of js/library.js serving library-detail-lab.html, so the detail
   takeover can be reworked without the live library page moving at all. The
   shelves, the data and the panel contents are identical; the only changes are
   in the open and close choreography, and they are marked LAB.

   The motion is reverse-engineered from video_references/reference_video.mp4:
   the shelf of Kurosawa cases where clicking one walks the camera into it and
   the case becomes the page. Frames were stepped at 60fps and the growing
   panel's top edge measured on each one, which gives a scale-over-time curve
   directly. A least-squares fit over those samples gives
   cubic-bezier(0.48, 0.04, 0.74, 0.84) to within one percent, and 460ms of
   geometry with the copy resolving about 150ms behind it. Those numbers live
   in css/library-detail-lab.css as --ease-dolly and --detail-open-ms.

   What that buys over what the site ships today:
     - one object, not two. The shipped version fades the whole overlay in on
       top of a shelf cover that is still sitting there, so a second cover
       dissolves into view over the first. Here the shelf cover is hidden and
       its double flies, opaque the whole way.
     - the shelf answers. It pushes away from the cover that was pressed on the
       same curve, so the page is entered rather than covered.
     - the cover turns. It leaves the shelf at an angle and squares up as it
       lands, which is the reference's most legible single gesture.
...
/* The platforms' own marks, as single-path logos, used to link out to each
   ...
   drawn from memory is a wrong logo. Discogs was exactly that for a while: the
   path opened with a rounded square that belongs to no part of the mark, and
   filled it over the record rings, so the one logo the comment was written to
   prevent was sitting underneath it. All four now match simple-icons. */
const SOURCE_MARK = {
...
function openDetail(item, node, { replace = false, fromHistory = false } = {}) {
  ...
  /* Two frames, not one. setMorphOrigin has just written the starting
     ...
     nothing to move from — the cover appears at full size instead of flying.
     The second frame guarantees the start state was painted. */
...
/* ============================================================
   LAB — the control panel

   Not part of the library page. It exists so the takeover can be compared
   against the motion the site ships without a reload, and slowed down enough
   that the curve is actually readable rather than just felt.
   ============================================================ */

function wireMotionLab() {
  const panel = document.querySelector(".motion-lab-panel");
  if (!panel) return;

  const root = document.documentElement;
  /* The unslowed values, read once before the slider has touched anything. */
  const base = {
    open: motionMs("--detail-open-ms", 460),
    close: motionMs("--detail-close-ms", 300),
  };
  const openOut = panel.querySelector("[data-fact-open]");
  const closeOut = panel.querySelector("[data-fact-close]");

  for (const button of panel.querySelectorAll("[data-motion]")) {
    button.addEventListener("click", () => {
      const legacy = button.dataset.motion === "legacy";
      document.body.classList.toggle("motion-legacy", legacy);
      for (const other of panel.querySelectorAll("[data-motion]")) {
        other.classList.toggle("is-on", other === button);
      }
      /* Both curves are worth seeing at the same speed, but the shipped one is
         hard-coded at 400ms and does not read the properties, so say so. */
      panel.classList.toggle("is-legacy", legacy);
    });
  }

  const slider = panel.querySelector("[data-speed]");
  const readout = panel.querySelector("[data-speed-out]");
  const apply = () => {
    const factor = Number(slider.value) / 100;
    const open = Math.round(base.open / factor);
    const close = Math.round(base.close / factor);
    /* Written to :root, where the stylesheet's calc() picks them up. Every
       duration and every delay in the takeover is a multiple of these two, so
       one write restages the whole sequence in proportion. */
    root.style.setProperty("--detail-open-ms", String(open));
    root.style.setProperty("--detail-close-ms", String(close));
    if (readout) readout.textContent = `${factor.toFixed(2)}×`;
    if (openOut) openOut.textContent = `${open}ms`;
    if (closeOut) closeOut.textContent = `${close}ms`;
  };
  slider?.addEventListener("input", apply);
  apply();

  /* Out of the way when it is the page being judged rather than the panel. */
...
if (document.querySelector(".motion-lab-panel")) wireMotionLab();
